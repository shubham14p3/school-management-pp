...
const App = () => {
  ...
  const handleLogin = (username, password) => {
    ...
    const adminUser = admins.find((user) => user.username === username && user.password === password);
    if (adminUser) {
      setLoggedIn(true);
    ...
    const staffUser = teachingStaff.find((user) => user.username === username && user.password === password);
    if (staffUser) {
      setLoggedIn(true);
    ...
    const studentUser = students.find((user) => user.username === username && user.password === password);
    if (studentUser) {
      setLoggedIn(true);
  ...
  return (
    <Router>
      <Routes>
        <Route path="/" element={loggedIn ? <Navigate to="/dashboard" /> : <LoginForm handleLogin={handleLogin} />} />
          ...
          element={
            loggedIn ? (
              userType === 'admin' ? (
                ...
                <TeachingStaffDashboard handleLogout={handleLogout} />
              ) : userType === 'student' ? (
        ...
        <Route path="/view-students" element={<ViewStudents students={students} />} when={loggedIn && userType === 'admin'} />

        {/* Route for adding a student */}
        <Route path="/add-student" element={<AddStudentForm students={students} setStudents={setStudents} />} when={loggedIn && userType === 'admin'} />

        {/* Route for viewing student notices */}
        <Route path="/student-notice" element={<StudentNotice studentNotices={studentNotices} />} when={loggedIn && userType === 'student'} />

        {/* Route for viewing teacher notices */}
        <Route path="/teacher-notice" element={<TeacherNotice teacherNotices={teacherNotices} />} when={loggedIn && userType === 'teachingStaff'} />

        {/* Route for leave management */}
        <Route path="/leave-management" element={<LeaveManagement />} when={loggedIn && userType === 'teachingStaff'} />

        {/* Route for task management */}
        <Route path="/task-management" element={<TaskManagement />} when={loggedIn && userType === 'teachingStaff'} />
